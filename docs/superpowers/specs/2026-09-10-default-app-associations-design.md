...
| Topic | Choice |
|---|---|
| Scope | All current Reader suffixes: `.docx` `.pptx` `.xlsx` `.md` `.pdf` `.json` `.yaml` `.yml` `.xml` `.c` `.h` `.txt` `.svg` `.png` `.jpg` `.jpeg` `.gif` `.webp` `.bmp` |
...
| Settings UI | Do **not** pop Settings when AssocQueryString already reports Reader. For UCPD-protected `.pdf` / `.txt` (UserChoice delete is Access denied), open `ms-settings:defaultapps?registeredAppUser=Reader` once and Invoke the missing rows so **SystemSettings.exe** writes the hashed UserChoice. Close Settings when finished. |
...
- Protected claim: skip Settings when `.pdf` / `.txt` already report Reader; open the Reader defaults URI and invoke missing protected suffixes; close Settings even if invoke fails
